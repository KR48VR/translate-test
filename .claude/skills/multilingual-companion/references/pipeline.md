...
0. Third-party source? `check_overlap.py --source <captured text> --master
   i18n/en.json` finds verbatim runs of 8+ words shared with the source —
   rewrite every flagged clause. Names, figures and official titles repeat
   legitimately; sentences must not. (Head-to-head testing caught a baseline
   agent copying 15-word runs while sincerely believing it had written an
   original retelling — run the script, don't trust the intention.)
